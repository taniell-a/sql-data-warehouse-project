/*
===============================================================================
Stored Procedure: Load Silver Layer (Bronze --> Silvver)
===============================================================================
Script Purpose:
    This stored procedure loads data into the 'silver' schema from bronze layer. 
    It performs the following actions:
    - Truncates the silver tables before loading data.
    - Uses the `INSERT` command to load cleaned and transformed data from Bronze to Silver tables.
	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE UPPER(LTRIM(RTRIM(prd_line)))
				...
				WHEN 'T' THEN 'Touring'
				ELSE 'N/A'
			END AS prd_line,		--Map procuct line codes to descriptive values 
		...
		SELECT
			...
			CASE 
				...
				ELSE sls_price				-- Derive price if original vale is invalid 
			END AS sls_price
		...
		SELECT 
			...
			CASE 
				...
				ELSE 'N/A'
			END AS gen			-- norlamilse gender values and handle unknown cases
		...
		SELECT
			...
			END AS cntry					--- Normalise and handle missing or blank values  
